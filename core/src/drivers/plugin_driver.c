...
void* plugin_thread_function(void *arg) {
    ...
    // Load the plugin
    if (!plugin_manager_load(plugin->manager)) {
        fprintf(stderr, "Failed to load plugin: %s\n", plugin->config.path);
        return NULL;
    }
    
    // TODO: Here you would generate structured args and call plugin init
    // Example:
    // plugin_runtime_args_t *args = generate_structured_args(driver, plugin->config.type);
    // if (args) {
    //     // Get init symbol from plugin
    //     typedef int (*plugin_init_func_t)(plugin_runtime_args_t *args);
    //     plugin_init_func_t init_func = dlsym(plugin_handle, "init");
    //     if (init_func) {
    //         init_func(args);
    //     }
...
static void plugin_runtime_args_capsule_destructor(PyObject *capsule) {
    plugin_runtime_args_t *args = (plugin_runtime_args_t *)PyCapsule_GetPointer(capsule, "openplc_runtime_args");
    if (args) {
        free_structured_args(args);
    }
}
...
static PyObject* create_python_runtime_args_capsule(plugin_runtime_args_t *args) {
    ...
    PyObject *capsule = PyCapsule_New(args, "openplc_runtime_args", plugin_runtime_args_capsule_destructor);
    if (!capsule) {
...
int plugin_driver_load_config(plugin_driver_t *driver, const char *config_file) {
    ...
    // Agora leio todos os simbolos que preciso (init, start, stop, cycle, cleanup) e adiciono na struct plugin_instance_t
    // para cada plugin.
...
int python_plugin_get_symbols(plugin_instance_t *plugin) {
    ...
    if (plugin_dir) {
        ...
        snprintf(python_path_cmd, sizeof(python_path_cmd), 
                "import sys; sys.path.insert(0, '%s')", dir_path);
    } else {
        snprintf(python_path_cmd, sizeof(python_path_cmd), 
    ...
    if (!py_binds->pModule) {
        fprintf(stderr, "Failed to load Python module '%s' from path '%s'\n", 
                module_name, plugin->config.path);
        PyErr_Print();
        free(py_binds);
        return -1;
    }
    ...
    if (!py_binds->pFuncInit || !PyCallable_Check(py_binds->pFuncInit)) {
        fprintf(stderr, "Error: 'init' function not found or not callable in module '%s' - this function is required\n", module_name);
    ...
    py_binds->pFuncCycleRun = PyObject_GetAttrString(py_binds->pModule, "run_cycle");
    if (!py_binds->pFuncCycleRun || !PyCallable_Check(py_binds->pFuncCycleRun)) {
        // run_cycle is optional
        Py_XDECREF(py_binds->pFuncCycleRun);
        py_binds->pFuncCycleRun = NULL;
    }
...
void python_plugin_cleanup(plugin_instance_t *plugin) {
    ...
    if (plugin && plugin->python_plugin) {
        ...
        Py_XDECREF(plugin->python_plugin->pFuncCycleRun);
